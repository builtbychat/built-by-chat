...
export function WorkloadPanel({ snapshot }: { snapshot: StudioSnapshot | null }) {
  ...
  return <article className="workload-card"><span>FOUR-WEEK HOST LOAD · PRIVATE</span><h2>{summary ? `${(summary.fourWeekMinutes / 60).toFixed(1)} hours recorded` : 'Loading history…'}</h2><p className="micro">Average stress {summary?.averageStress.toFixed(1) ?? '—'} / 5 · recovery {summary?.averageRecovery.toFixed(1) ?? '—'} / 5</p><form onSubmit={(event) => void save(event)}><div className="workload-fields">{(['prepMinutes','liveMinutes','postMinutes','adminMinutes'] as const).map(key => <label key={key}>{key.replace('Minutes',' minutes')}<input name={key} type="number" min="0" max="10080" value={form[key]} onChange={event => setForm({...form,[key]:Number(event.target.value)})} /></label>)}<label>Stress (1–5)<input name="stress" type="number" min="1" max="5" value={form.stress} onChange={event => setForm({...form,stress:Number(event.target.value)})} /></label><label>Recovery (1–5)<input name="recovery" type="number" min="1" max="5" value={form.recovery} onChange={event => setForm({...form,recovery:Number(event.target.value)})} /></label></div><label>Private recovery note<textarea name="recovery-notes" maxLength={1000} value={form.notes} onChange={event => setForm({...form,notes:event.target.value})} /></label><button className="button secondary" disabled={!snapshot}>Save workload check-in</button><p className="micro" aria-live="polite">{message}</p></form></article>;
}
